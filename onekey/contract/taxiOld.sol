...
contract Taxi
{
	...
	function driverSelction(int x, int y, uint orderIndex) private returns(bool)
	{
		uint i;
		uint j;
		int threshold = 50000; 	//阀值，当距离小于该值之后则派单，数值可调整
		int temp;
	...
	function calculatePreFeeWithPenatly(int s_x, int s_y, int d_x, int d_y) returns(int)
	{
		int tempX = s_x - d_x;
		int tempY = s_y - d_y;
		if (tempX < 0)
		...
		return ((tempX + tempY) * unitPrice) / 2 * 3 / 100 + 500;
	}
	...
	{
		return counterOrderIndex;
	}

	function tsTotalNumOfDriver() returns(uint)
	{
		return counterDriverIndex;
	}

	function tsDriverInfo(uint driverIndex) returns(bool isworking, int x, int y, uint state, int totalJudge, int avgScore, address name, string info0, string info1, string info2)
	{
		isworking = drivers[driverIndex].state;
		x = drivers[driverIndex].cor_x;
		y = drivers[driverIndex].cor_y;
		state = driverStates[drivers[driverIndex].name];
		name = drivers[driverIndex].name;
		info0 = drivers[driverIndex].info0;
        info1 = drivers[driverIndex].info1;
        info2 = drivers[driverIndex].info2;
        totalJudge = driverJudgements[drivers[driverIndex].name].total;
        avgScore = driverJudgements[drivers[driverIndex].name].avgScore;
	}
